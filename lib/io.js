...
function getFilepath(filename, extension) {
	...
	const filepathPrefix1 = (new Date()).getTime();
	const filepathPrefix2 = process.env.DB_API_TOKEN.substring(0, 5);

	return path.join(filepathFolder, filepathPrefix1 + '_' + filepathPrefix2 + '_' + filename + '.' + extension);
}
