import { map, extend, pick, sortBy, each, omit } from 'underscore';
			...
			switch (key) {
				case 'cookies':
					for (let c = 0; c < o[key].length; c++) {
						result[camelCase(prefix + 'cookie_' + o[key][c].name)] =
...
function flattenObjects(iterable) {
	return map(iterable, (item) => flattenObject(item));
}
...
function flattenTrackingData(trackingData, fields = null) {
	...
	if (
		...
		for (let i = 0; i < cookiesLength; i++) {
			trackingData['cookie_' + trackingData.config.cookies[i].name] =
	...
	const pickedPropItem = extend({}, pick(trackingData, ...fields));
	if (fields.includes('headers')) {
		extend(
			pickedPropItem,
			pick(trackingData, (value, key) => {
...
function flattenEachTrackingData(iterableOftrackingData) {
	return map(iterableOftrackingData, (item) => flattenTrackingData(item));
}
...
function flattenScenariosReportSummariesData(scenariosReports) {
	...
	return sortBy(summaryData, 'timestamp');
}
